...
#ifndef __gcore_rexp_h_
#define __gcore_rexp_h_

#include <gcore/config.h>

// Ignore Escape Character Macro
#define IEC(str) ((const char*)gcore::_Exp(#str))
...
namespace gcore {
  ...
  class GCORE_API MatchData {
    
    public:
        
      friend class Regexp;
      
      MatchData(int dim, int mode=REX_FORWARD, int from=0, int to=2147483647);
      ~MatchData();
      ...
      const std::string get(const std::string &str, int i) const;
  ...
  struct GCORE_API _Exp {
    _Exp(const char *s):e(s){e.erase(0,1);e.erase(e.length()-1,1);}
  ...
  class GCORE_API Regexp {
    public:
      ...
      static const char* getError(RexError err){ return errors[err]; }
