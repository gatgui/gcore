...
Status InvalidValue::fromXML(const XMLElement *)
{
   return Status(false, "Invalid value.");
}

XMLElement* InvalidValue::toXML(XMLElement *) const
{
   return NULL;
}

Value* InvalidValue::clone() const
{
...
const gcore::String String::DefaultValue("");
...
String::String(const gcore::String &str)
   : mValue(str)
{
   mNull = false;
   mType = PropertyList::ValueTypeID("string");
...
Status String::fromXML(const XMLElement *elt)
{
   if (!elt)
   {
...
XMLElement* String::toXML(XMLElement *elt) const
{
   if (!elt)
   {
      elt = new XMLElement("string");
   }
   elt->setText(mValue);
   return elt;
}
...
const long Integer::DefaultValue = 0;
...
Status Integer::fromXML(const XMLElement *elt)
{
   if (!elt)
   {
      return Status(false, "NULL element.");
...
XMLElement* Integer::toXML(XMLElement *elt) const
{
   if (!elt)
   {
      elt = new XMLElement("integer");
   }
...
const double Real::DefaultValue = 0.0;
...
Status Real::fromXML(const XMLElement *elt)
{
   if (!elt)
   {
      return Status(false, "NULL element.");
...
{
   return new Real(mValue);
}

XMLElement* Real::toXML(XMLElement *elt) const
{
   if (!elt)
   {
      elt = new XMLElement("real");
   }
   elt->setText(gcore::String(mValue));
   return elt;
}
...
const bool Boolean::DefaultValue = false;
...
Status Boolean::fromXML(const XMLElement *elt)
{
   if (!elt)
   {
      return Status(false, "NULL element.");
...
XMLElement* Boolean::toXML(XMLElement *elt) const
{
   if (!elt)
   {
      //elt = new XMLElement("boolean");
      if (mValue)
      {
         elt = new XMLElement("true");
      }
      else
      {
         elt = new XMLElement("false");
      }
   }
   else
   {
...
const List<Value*> Array::DefaultValue;
...
Array::Array(const List<Value*> &val)
   : mValues(val)
{
   mNull = false;
   mType = PropertyList::ValueTypeID("array");
...
Value* Array::at(size_t idx)
{
   static InvalidValue dummy = InvalidValue();
   if (idx >= mValues.size())
   {
      return &dummy;
   }
...
const Value* Array::at(size_t idx) const
{
   static InvalidValue dummy = InvalidValue();
   if (idx >= mValues.size())
   {
...
void Array::set(size_t idx, Value *v, bool replace)
{
   if (idx < mValues.size())
   {
...
Status Array::fromXML(const XMLElement *elt)
{
   clear();
   if (!elt)
   {
...
XMLElement* Array::toXML(XMLElement *elt) const
{
   if (!elt)
   {
      elt = new XMLElement("array");
   }
   for (size_t i=0; i<mValues.size(); ++i)
   {
      if (mValues[i])
...
const std::map<gcore::String, Value*> Dictionary::DefaultValue;
...
Dictionary::Dictionary(const std::map<gcore::String, Value*> &val)
   : mPairs(val)
{
   mNull = false;
   mType = PropertyList::ValueTypeID("dict");
...
bool Dictionary::has(const gcore::String &key) const
{
   return (mPairs.find(key) != mPairs.end());
}

size_t Dictionary::keys(StringList &keys) const
{
    keys.clear();
...
Value* Dictionary::value(const gcore::String &key)
{
   static InvalidValue dummy = InvalidValue();
...
const Value* Dictionary::value(const gcore::String &key) const
{
   static InvalidValue dummy = InvalidValue();
...
void Dictionary::set(const gcore::String &key, Value *v, bool replace)
{
   std::map<gcore::String, Value*>::iterator it = mPairs.find(key);
   if (it != mPairs.end())
   {
...
Status Dictionary::fromXML(const XMLElement *elt)
{
   clear();
   if (!elt)
   {
...
XMLElement* Dictionary::toXML(XMLElement *elt) const
{
   if (elt == NULL)
   {
   ...
   while (it != mPairs.end())
   {
      if (it->second)
      {
         XMLElement *k = new XMLElement("key");
         k->setText(it->first);
         elt->addChild(k);
...
const char* Dictionary::TypeName()
{
   return "dict";
}
