...
int main(int argc, char **argv) {
  ...
  std::map<std::string, gcore::Base85::CharSet> csid;
  std::map<std::string, gcore::Base85::CharSet>::iterator it;
  ...
  csid["ascii85"] = gcore::Base85::CS_ASCII85;
  csid["z85"] = gcore::Base85::CS_Z85;
  csid["ipv6"] = gcore::Base85::CS_IPV6;
  csid["arnold"] = gcore::Base85::CS_SA;
  
  gcore::Base85::CharSet cs = gcore::Base85::CS_ASCII85;
  std::string encstr = "";
  std::string decstr = "";
  ...
  for (int a=1; a<argc; ++a) {
    if (!strcmp(argv[a], "-cs")) {
      ++a;
      if (a >= argc) {
        std::cout << "invalid arguments (-cs ascii85|z85|ipv6|arnold, -e <str> or -d <str> allowed)" << std::endl;
        return 1;
      }
      it = csid.find(argv[a]);
      if (it != csid.end()) {
  ...
  es = gcore::Base85::Encode(&indata[0], inlen, cs);
  
  std::cout << "Encode floats: \"" << es << "\"" << std::endl;
  
  std::cout << es << std::endl;
  if (gcore::Base85::Decode(es, outdata, outlen, cs)) {
  ...
  if (encstr.length() > 0) {
    ...
    if (gcore::Base85::Decode(es, outdata, outlen, cs)) {
      std::cout << "\"" << es << "\" => \"" << (const char*)outdata << "\" (" << outlen << ")" << std::endl;
      free(outdata);
    }
  }
  ...
  if (decstr.length() > 0) {
    ...
    if (!gcore::Base85::Decode(decstr, outstr, cs)) {
      std::cout << "Failed" << std::endl;
    
    } else {
      std::cout << "\"" << decstr << "\" => - ";
      for (size_t i=0; i<outstr.length(); ++i) {
        std::cout << std::hex << int((unsigned char)outstr[i]) << std::dec << " - ";
      }
      std::cout << std::endl;
      
      es = gcore::Base85::Encode(outstr, outlen, cs);
      std::cout << "=> \"" << es << "\"" << std::endl;
    }
  }
  
  // arnold encoding tests
  cs = gcore::Base85::CS_SA;
  ...
  decstr = "!%<_l4$$$$d"; // original string prefixed with 'B'
  if (gcore::Base85::Decode(decstr, outdata, outlen, cs, 4)) {
  ...
  decstr = "!%<_l4$$$$'$vDc3$v;Z0$v;Z0$u"; // original string prefixed with 'B'
  if (gcore::Base85::Decode(decstr, outdata, outlen, cs, 4)) {
    unsigned int *vals = (unsigned int*)outdata;
    unsigned int count = outlen / 4;
    std::cout << "Decoded " << count << " values" << std::endl;
  ...
  decstr = "!%<_l4$$&<:%<"; // original string prefixed with 'B'
  if (gcore::Base85::Decode(decstr, outdata, outlen, cs, 4)) {
    unsigned int *vals = (unsigned int*)outdata;
    unsigned int count = outlen / 4;
    std::cout << "Decoded " << count << " values" << std::endl;
  ...
  // Original string prefixed with 'F'
  // => Count is not correct (1026 instead of expected 1024, values are correct)
  if (gcore::Base85::Decode(decstr, outdata, outlen, cs, 3)) {
    unsigned int *vals = (unsigned int*)outdata;
    unsigned int count = outlen / 4;
  ...
  // Original string prefixed with 'C'
  if (gcore::Base85::Decode(decstr, outdata, outlen, cs, 2)) {
    unsigned int *vals = (unsigned int*)outdata;
    unsigned int count = outlen / 4;
    std::cout << "Decoded " << count << " indices" << std::endl;
    std::cout << "  [0] " << vals[0] << std::endl;
  ...
  outlen = 0;
  // indices in [0-66049]
  decstr = "$'G:D$$HT0$'G=F$$$<,.C&u3$$$<,.ImIr";
  // Should starts with 0, 1, 258, 257, 1, 2, 259, 258, 2, 3, 260
  // Original string prefixed with 'G'
  // 'G' -> dup 1? not really seems to be more of a 17 bit shift (remaining bits? as with 10 bits shift)
  if (gcore::Base85::Decode(decstr, outdata, outlen, cs, 1)) {
    unsigned int *vals = (unsigned int*)outdata;
    unsigned int count = outlen / 4;
    std::cout << "Decoded " << count << " indices" << std::endl;
    std::cout << "  [0] " << vals[0] << std::endl;
  ...
  if (gcore::Base85::Decode(decstr, outdata, outlen, cs)) {
