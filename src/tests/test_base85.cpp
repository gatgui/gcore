...
int main(int argc, char **argv) {
  ...
  es = gcore::Base85::Encode(enc, &indata[0], inlen); //, cs);
  ...
  if (encstr.length() > 0) {
    ...
    //if (gcore::Base85::Decode(es, outdata, outlen, cs)) {
    //if (gcore::Base85::Decode(dec, es, outdata, outlen)) {
    if (gcore::Base85::Decode(dec, es, out)) {
      // Decode is decoding a data bloack, doesn't include final '\0'
      //std::cout << "\"" << es << "\" => \"" << (const char*)outdata << "\" (" << outlen << ")" << std::endl;
      //free(outdata);
      std::cout << "\"" << es << "\" => \"" << out << "\" (" << out.length() << ")" << std::endl;
    }
  }
  ...
  if (decstr.length() > 0) {
    ...
    if (!gcore::Base85::Decode(dec, decstr, outstr)) {
      ...
      es = gcore::Base85::Encode(enc, outstr, outlen);
      std::cout << "=> \"" << es << "\"" << std::endl;
    }
  }
